...
- (id)initWithPhotoArray:(NSArray *)array
{
    self = [self init];
    if (self)
    {
...
- (CXBrowserNavBarView *)browserNavigationBarViewOfOfPhotoBrowser:(CXPhotoBrowser *)photoBrowser withSize:(CGSize)size
{
    ...
    if (!navBarView)
    {
        ...
        UIView *bkgView = [[UIView alloc] initWithFrame:CGRectMake( 0, 0, size.width, size.height + 20)];
        [bkgView setBackgroundColor:[UIColor whiteColor]];
        bkgView.alpha = 0.9;
        ...
        [doneButton setTitleColor:[UIColor colorWithRed:1 green:1 blue:1 alpha:1] forState:UIControlStateHighlighted];
        [doneButton setTitle:@"分享" forState:UIControlStateNormal];
        [doneButton setFrame:CGRectMake(size.width - 55, 25, 55, 40)];
        [doneButton addTarget:self action:@selector(photoBrowserDidTapShareButton:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [doneButton setFrame:CGRectMake(0, 25, 55, 40)];
        ...
        [titleLabel setFrame:CGRectMake((size.width - 100)/2, 20, 100, 40)];
...
- (CXBrowserToolBarView *)browserToolBarViewOfPhotoBrowser:(CXPhotoBrowser *)photoBrowser withSize:(CGSize)size
{
    ...
    if (!toolBarView)
    {
        ...
        [bkgImageView setBackgroundColor:[UIColor blackColor]];
        bkgImageView.alpha = 0.3;
        ...
        [likeButton setTitle:NSLocalizedString(@"保存",@"") forState:UIControlStateNormal];
...
- (void)photoBrowserDidTapShareButton:(UIButton *)sender
{
    CXPhoto * cxPhoto= [photosArray objectAtIndex:self.currentPageIndex];
    NSArray *activityItems = [[NSArray alloc]initWithObjects:cxPhoto.underlyingImage, nil];
...
- (void)resetCustomlizeBrowserNavigationBarView
{
    ...
    {
        browserNavigationBarView = [self browserNavigationBarViewOfOfPhotoBrowser:self withSize:frame.size];
    }
...
- (void)resetCustomlizeBrowserToolBarView
{
    ...
    browserToolBarView = [self browserToolBarViewOfPhotoBrowser:self withSize:frame.size];
...
- (void)currentPageDidUpdated
{
    [self photoBrowser:self didChangedToPageAtIndex:_currentPageIndex];
...
- (void)reloadData {
    ...
    _supportReload = [self supportReload];
...
- (id<CXPhotoProtocol>)photoAtIndex:(NSUInteger)index
{
    ...
    if (index < _photos.count) {
        if ([_photos objectAtIndex:index] == [NSNull null]) {
            photo = [photosArray objectAtIndex:index];
